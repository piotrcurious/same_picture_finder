...
def is_already_renamed(filename):
    # Check if already starts with seq_XXX_YYY_
    return re.match(rf"^{SEQUENCE_PREFIX}_\d{{3}}_\d{{3}}_", filename)
...
def check_similarity_align_image_stack(img1_path, img2_path):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        command = [
            'align_image_stack',
            '-p', f"{temp_output_prefix}.pto",   # PTO project file
            '-o', temp_output_prefix,             # output image prefix
            img1_path,
            img2_path
        ]
        try:
...
def rename_images(directory):
    ...
    for seq_idx, seq in enumerate(sequences, 1):
        ...
        for img_idx, img_name in enumerate(seq, 1):
            ...
            new_name = f"{SEQUENCE_PREFIX}_{seq_idx:03d}_{img_idx:03d}_{img_name}"
